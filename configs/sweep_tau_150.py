...
def get_config() -> SimpleNamespace:
    """
    Sweep point: tau x1.5, dirichlet at v4 (unscaled), c_puct fixed.

    Follow-up to the 42 (tau x1.25) vs 50 (clean v4, tau x1.0) result: 42 won 100-0-0, the
    cleanest result in the sweep. This point tests whether pushing tau further keeps helping
    or whether 1.25x is near a ceiling. See configs/sweep_tau_075.py for the general sweep
    design notes and docs/current/SWEEP_TAU_DIRICHLET.md for the full story.
    """
    return SimpleNamespace(
        seed=12345,
        cycle=61,  # placeholder; actual cycle id is always passed via --cycle / CYCLE=
        ...
        tau_cutoff_plies=3,
        tau_early=0.15,
        tau_early_plies={0: 1.17, 1: 0.69, 2: 0.42},  # v4 x1.5
        add_dirichlet_noise=True,
        dirichlet_epsilon=0.10,  # v4, unscaled (this axis's center)
