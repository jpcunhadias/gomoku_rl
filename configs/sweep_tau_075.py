...
def get_config() -> SimpleNamespace:
    """
    Sweep point: tau x0.75, dirichlet at v4 (unscaled), c_puct fixed.

    Base is Cycle 2's v4 config (configs/phaseC_c2.py) with tau_early_plies scaled by 0.75.
    Self-play starts from the Cycle 2 checkpoint (v4 IS the 1.0x point for this axis, so it's
    reused rather than rerun — see docs/CHANGELOG.md's sweep section).

    num_self_play_games reduced 200 -> 100 for a faster first-pass sweep.
    """
    return SimpleNamespace(
        seed=12345,
        cycle=31,  # placeholder; actual cycle id is always passed via --cycle / CYCLE=
        ...
        tau_cutoff_plies=3,
        tau_early=0.15,
        tau_early_plies={0: 0.585, 1: 0.345, 2: 0.21},  # v4 x0.75
