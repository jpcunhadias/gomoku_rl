...
def test_policy_value_net_shape_and_value_range():
    ...
    # Test forward pass with random input (1, 3, 15, 15)
    dummy_input = torch.rand(1, 3, 15, 15)  # Batch size of 1
...
def test_residual_block():
    ...
    x = torch.rand(1, 64, 15, 15)  # random input tensor
    out = block(x)
...
def test_fixed_input():
    ...
    fixed_board = torch.zeros(1, 3, 15, 15)
    fixed_board[0, 0, 0, 0] = 1  # Player 1's piece at top-left
    fixed_board[0, 1, 14, 14] = 1  # Player 2's piece at bottom-right
